...
def run_full_finetuning():
    ...
    transformer = SD3Transformer2DModel.from_pretrained("stabilityai/stable-diffusion-3.5-large", subfolder="transformer")
    text_encoder = CLIPTextModel.from_pretrained("stabilityai/stable-diffusion-3.5-large", subfolder="text_encoder")
    noise_scheduler = FlowMatchEulerDiscreteScheduler.from_pretrained("stabilityai/stable-diffusion-3.5-large", subfolder="scheduler")
    ...
    for step in range(5000):
        ...
        # AI가 예측한 결과물 출력
        model_pred = transformer(mock_images, timestep=timesteps, encoder_hidden_states=encoder_hidden_states).sample
